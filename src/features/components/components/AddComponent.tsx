...
import { useAddComponent, useGetAssets, useGetComponentCategory, useUpdateComponent } from '../api';
import { type IComponentForm, type IComponent } from '../types';

export const AddComponent = ({ isModal = false }: IsModal) => {
  const { enqueueSnackbar } = useSnackbar();
  const methods = useForm<IComponentForm>();
  const { handleSubmit, setError, reset, setValue } = methods;
  const addComponent = useAddComponent<IComponent>();
  const updateComponent = useUpdateComponent<IComponent>();
  const { data: manufacturerOptions } = useGetManufacturer();
  const { data: categoryOptions } = useGetComponentCategory();
  ...
  const { data: component, refetch } = useGetAssetsDataById<IComponent>(
    Number(id),
    apiUrl.components + '/' + id,
  ...
  const setValues = useCallback(
    (assetValues: any) => {
      ...
      const categoryObject = categoryOptions?.data.find((option) => {
        return option.id === assetValues.asset_component_category_id;
      });
      ...
      const assetObject = assets?.data.find((option) => option.id === assetValues.asset_id);
      setValue('asset_id', assetObject !== undefined ? assetObject : null);
    },
    [assets?.data, categoryOptions?.data, manufacturerOptions, setValue],
  );
  ...
  }, [component, id, isIdNotValid, location.pathname, navigate, refetch, reset, setValues]);

  const onSubmit = async (data: IComponentForm) => {
    const tempData = {
      name: data.name,
      serial: data.serial,
      manufacturer_id: data.manufacturer_id?.id ? data.manufacturer_id.id : null,
      asset_component_category_id: data.asset_component_category_id?.id
        ? data.asset_component_category_id.id
        : null,
      asset_id: data.asset_id?.id ? data.asset_id.id : null,
    };

    if (action === 'Add')
      addComponent.mutate(tempData, {
        onSuccess: () => {
          const variant = getVariant('success');
          enqueueSnackbar('Component has been added', { variant });
          reset();
        },
        onError(error) {
    ...
    if (action === 'Edit' && id)
      updateComponent.mutate(
        { id: id, body: tempData },
        {
          onSuccess: () => {
            const variant = getVariant('success');
            enqueueSnackbar('Component has been edited', { variant });
            navigate(routePath.components);
          },
          onError(error) {
            const e: { message: string } = error.response?.data as { message: string };
            setError('name', { type: 'server', message: e.message }, { shouldFocus: false });
          ...
          <FormProvider {...methods}>
            ...
            >
              <Grid alignContent="center" container display="flex" item mt={2} spacing={2}>
                ...
                <SelectInput
                  label="Category"
                  name="asset_component_category_id"
                ...
                <SelectInput
                  ...
                  name="asset_id"
                  options={assets?.data ? assets.data : []}
                  createButton={false}
                />
                <Grid
                  alignContent="center"
